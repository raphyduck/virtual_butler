 ...
 *    moment they scroll upward — preventing any auto-scroll from interrupting them.
 *
 * Usage
 * -----
...
export function useScrollAnchor(): ScrollAnchor {
  ...
  /** Attach / re-attach the scroll listener whenever the container changes. */
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;
  ...
  const scrollToBottom = useCallback((behavior: ScrollBehavior = "smooth") => {
    ...
    // Also immediately mark as "at bottom" so any follow-up effects don't
    // double-scroll before the browser's paint completes.
    setIsAtBottom(true);
  }, []);
