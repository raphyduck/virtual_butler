...
async def _get_owned_ability(ability_id: uuid.UUID, user_id: uuid.UUID, db: AsyncSession) -> Ability:
    result = await db.execute(select(Ability).where(Ability.id == ability_id, Ability.user_id == user_id))
